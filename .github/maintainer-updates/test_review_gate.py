...
class CheckGateTests(unittest.TestCase):
    ...
    def test_rejects_failed_or_cancelled_checks(self):
        for bucket in ("fail", "cancel"):
            with self.subTest(bucket=bucket), self.assertRaises(
                review_gate.CheckFailure
            ):
                review_gate.wait_for_passing_checks(
                    lambda: [check("build", bucket, "FAILURE")],
                    interval_seconds=0,
                    max_attempts=1,
...
class GitHubCliTests(unittest.TestCase):
    def test_finds_the_single_open_pull_request_for_the_exact_head(self):
        ...
        pull_request = cli.find_open_pull_request(
            "TyceHerrman:auto-update/whatcable-1.4.0"
        )
        ...
        self.assertEqual(
            ...
            [
                ...
                "state=open",
                "-f",
                "head=TyceHerrman:auto-update/whatcable-1.4.0",
                "-f",
                "per_page=100",
            ],
        )
    ...
    def test_lists_review_runs_for_deduplication(self):
        ...
        cli = review_gate.GitHubCli("dispatch-token", runner=runner)
        ...
        self.assertEqual(
            ...
            [
                ...
                "list",
                "--repo",
                "TyceHerrman/nixpkgs-review-gha",
                "--workflow",
                "review.yml",
                "--event",
                "workflow_dispatch",
